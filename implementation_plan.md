...
## 背景と目的
OpenFisca-Japan を直接呼び出すには、データのベクトル化（ndarray化）や内部の人物・世帯エンティティへの紐付けなど複雑な処理が必要です。この複雑な部分を隠蔽し、使いやすい JSON ベースの API としてアクセスできるようにするツール (`get_tax_benefit_info`, `calc`) を提供します。これにより MCP 経由での LLM からの呼び出しや、Python SDK としての利用が容易になります。
...
1. **`get_tax_benefit_info(tax_benefit_name: str) -> list[dict]`**
...
- `@mcp.tool()` デコレータを用いて、`get_tax_benefit_info` と `calc` を登録します。
...
- ダミーデータを利用して `calc` および `get_tax_benefit_info` を実行し、想定通りの形で JSON 辞書が返却されるかを確認する簡単なテスト用スクリプトを作成・実行します。
